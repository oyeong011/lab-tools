...
cd ~/lab/_drive_stage/lab-acceptance-bundles
lab-acceptance-matrix --config required-hosts.json --bundle-dir .
...
Acceptance bundle은 `.tar.gz`와 `.tar.gz.sha256` 두 파일을 같이 보존해야 합니다. 공간이 부족하면 `lab-acceptance-stage --out <Google-Drive-folder>`로 최신 통과 bundle과 검증 config만 골라 옮기면 되고, 받은 컴퓨터에서는 `lab-acceptance-import <Google-Drive-folder>`로 `~/lab/_acceptance_bundles`에 가져온 뒤 matrix를 다시 확인하면 됩니다.
...
| Drive/USB용 bundle 스테이징 | `lab-acceptance-stage --out <folder>` |
| Drive/USB stage 가져오기 | `lab-acceptance-import <folder>` |
