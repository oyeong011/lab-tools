...
Acceptance bundle은 `.tar.gz`와 `.tar.gz.sha256` 두 파일만 같이 보존하면 됩니다. 공간이 부족하면 `lab-acceptance-stage --out <Google-Drive-folder>`로 최신 통과 bundle만 골라 옮기면 되고, 다시 검사할 때는 그 폴더를 `lab-acceptance-matrix --bundle-dir <Google-Drive-folder>`에 넘기면 됩니다.
